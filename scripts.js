...
const handleSettingsSubmit = (event) => {
  ...
  document.documentElement.style.setProperty(
    "--color-dark",
    isNightMode ? "255, 255, 255" : "10, 10, 20"
  );
  document.documentElement.style.setProperty(
    "--color-light",
    isNightMode ? "10, 10, 20" : "255, 255, 255"
  );
...
const handleBookClick = (event) => {
  ...
  const active = pathArray.find(
    (node) =>
      node?.dataset?.preview &&
      books.find((book) => book.id === node.dataset.preview)
  );
...
const displayBookDetails = (book) => {
  ...
  selectors.listSubtitle.innerText = `${authors[book.author]} (${new Date(
    book.published
  ).getFullYear()})`;
